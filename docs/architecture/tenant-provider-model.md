...
Our inference token billing implementation supports two
tenant identification strategies:

1. **Namespace-based (default):** Uses namespace as the
   tenant dimension. When a tenant provider is configured,
   it resolves namespace → tenant_id automatically.

2. **Identity-based (Sovereign Gateway Pattern):** When
   the `organization` field is populated in the token
   billing data (via Keycloak JWT → Authorino →
   TelemetryPolicy pipeline), the organization IS the
   OSAC tenant — no additional tenant resolution layer
   is needed. The `organization` column maps directly to
   OSAC Organizations / Keycloak realms.

   This means for deployments using Red Hat Connectivity
   Link with identity-based metrics, the OSAC tenant
   provider described above is effectively built into the
   data itself. Reports use `group_by[organization]` to
   get per-tenant costs without any tenant mapping
   configuration.
